...
输入格式
两个数组，分别用两行表示，第一行表示数组 students和第二行表示sandwiches，中间用逗号隔开
输出格式
输出为一个数字，表示为无法吃午餐的学生数量
...
3
说明/提示
输入：students = [1,1,0,0], sandwiches = [0,1,0,1] 输出：0 解释：
...
for i in range(1000):
    ...
    for j in range(len(students)):
        # count += 1
        if sandwiches[0] == students[j]:
            print(2222222)
            print(students[j])
            myls.remove(sandwiches[0])
            if len(myls) == 0:
                break
            print(students)
            print(myls)
